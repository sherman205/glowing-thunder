...
def get_activities_data(start_date, end_date, activity_type=None):
	...
	cache_key = Cache.athlete_activities_cache_key
	last_synced_cache_key = Cache.last_synced_cache_key

	cache_last_synced = cache.get(last_synced_cache_key)
	if cache_last_synced:
		cache_last_synced = cache_last_synced - timedelta(hours=12)
		# Call api here with `after` param to fetch recent activity data

	cached_athlete_activities = cache.get(cache_key) or []
	if not cached_athlete_activities or not cache_last_synced:
		# Remove below logic, temporary
		# Call api here to retrieve all activities with no params
		cache.set(cache_key, athlete_activities, None)
		cached_athlete_activities = cache.get(cache_key)

	date_data_map = defaultdict(lambda: defaultdict(int))
	activity_types = set()
	for activity in cached_athlete_activities:
		...
		date_data_map[date_timestamp]['suffer_score'] += activity.get('suffer_score')
